...
def find_supported_versions(plugin_dir: str, only_min: bool = False, include_prerelease: bool = False) -> List[str]:
  ...
  if include_prerelease:
    ...
    prereleases = [release for release in gh_releases if release["name"].startswith("core/") and release["prerelease"]]
  ...
  for folder in folders:
    ...
    if folder != "core":
      ...
      if pyproj_version != core_version:
        errors.append(f"bodhilib version `{pyproj_version}` in {folder} does not match core version `{core_version}`")
  if errors:
    print("\n".join(errors))
    return 1
  return 0
